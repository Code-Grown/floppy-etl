...
def main( args_bag: Any ) -> None:
    transform_example(args_bag)

def transform_example(args_bag: Any) -> None:
    date_yesterday = datetime.strftime(datetime.now() - timedelta(days=1), '%Y-%m-%d')
    log_hint = '[transform_example]'
    Log.info(f'Example: {log_hint} transforming data from yesterday -> : {date_yesterday}.')
